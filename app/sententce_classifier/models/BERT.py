...
from .models import BertSequenceClassifier, DistilBertSequenceClassifier
...
class BERTForSentenceClassification(object):
    ...
    def from_bert_model(cls,
        ...
        # * Config *
        architecture_config = BertConfig.from_pretrained(bert_model)
        architecture_config.__setattr__('num_labels', output_dim)
        # save the config file

        # * MODEL *
        # Set-up the model
        model = BertSequenceClassifier.from_pretrained(bert_model, config=architecture_config)
    ...
    @classmethod
    def from_german_bert(cls,
                         model_directory: Path,
                         bert_model: (str,) = german_bert_model,
                         device: str = CHOSEN_DEVICE,
                         *args, **kwargs,
                         ):
        ...
        # * Config *
        architecture_config_class = BertConfig
        architecture_config = architecture_config_class.from_json_file(os.path.join(model_directory, 'config.json'))
        model = BertSequenceClassifier(architecture_config)
        ...
        tokenizer = BertTokenizer(vocab_file=vocab_file, **config_tokenizer)
    ...
    def _save_model(self,
        ...
        """
        filename_pth = os.path.join(self.model_directory, f"{self.bert_model.replace('/', '-')}_model_{self.epoch}.pth")  #TODO
